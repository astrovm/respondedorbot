...
pub fn log_bot_config() {
    ...
    CONFIG_LOGGED.get_or_init(|| match config::load_bot_config() {
        Ok(cfg) => {
...
fn response(status: StatusCode, body: impl Into<String>) -> WebhookResponse {
    WebhookResponse {
        status,
        body: body.into(),
    }
...
pub async fn handle_get(state: &AppState, query: WebhookQuery) -> WebhookResponse {
    let Some(key) = query.key else {
        return response(StatusCode::OK, "No key");
    };
    ...
    if is_true(&query.update_dollars) {
        let updated = market::get_dollar_rates(&state.http, &state.redis)
            ...
            .is_some();
        if updated {
            return response(StatusCode::OK, "Dollars updated");
        }
        return response(StatusCode::BAD_REQUEST, "Dollars update error");
    }
...
pub async fn handle_post(
    ...
    let update = match update {
        Some(value) => value,
        None => return response(StatusCode::BAD_REQUEST, "Invalid JSON"),
    };
...
async fn handle_check_webhook(state: &AppState) -> WebhookResponse {
    ...
    match telegram::get_webhook_info(&state.http, token).await {
        Ok(info) => {
            ...
            if current == expected {
                response(StatusCode::OK, "Webhook checked")
            } else {
                response(StatusCode::BAD_REQUEST, "Webhook check error")
            }
        }
        Err(_) => response(StatusCode::BAD_REQUEST, "Webhook check error"),
    }
}

async fn handle_update_webhook(state: &AppState) -> WebhookResponse {
    let Some(token) = state.telegram_token.as_deref() else {
...
async fn is_secret_token_valid(state: &AppState, secret_header: Option<&str>) -> bool {
