    ...
    *,
    ...
    for market in event.get("markets") or []:
        if not isinstance(market, dict):
            continue
        if active_only and (market.get("active") is False or market.get("closed") is True):
            continue
        ...
        try:
            ...
            continue
        title = market.get("groupItemTitle") or market.get("question") or market.get("slug")
        if not title:
            continue
        token_id = (
...
def format_event_section(
    ...
    *,
    ...
    for quote in quotes:
        ...
        odds.append((quote.title, max(0.0, min(probability, 1.0)) * 100))
    ...
    filtered = [
        item
        for item in odds
        if any(item[0].strip().upper().startswith(prefix.upper()) for prefix in filter_prefixes)
    ]
    lines = [header, ""]
...
def event_country_flag(event: dict[str, Any]) -> str:
    for tag in event.get("tags") or []:
        flag = country_flag_from_name(str(tag.get("slug") or "")) if isinstance(tag, dict) else ""
        if flag:
            return flag
    return ""
...
def get_global_elections(
    *,
    ...
    events = response.get("data") if response else None
    if not isinstance(events, list) or not events:
        return tr("market.polymarket.error")

    events.sort(key=lambda event: float(event.get("liquidity") or 0), reverse=True)
    selected_events = events[:GLOBAL_ELECTIONS_LIMIT]
    normalized_events = [(event, normalize_event_quotes(event)) for event in selected_events]
    token_ids = [
        quote.token_id for _event, quotes in normalized_events for quote in quotes if quote.token_id
    ]
    ...
    lines = [tr("market.polymarket.title")]
    for event, quotes in normalized_events:
        ...
        for outcome_title, probability in _top_outcomes(
            ...
            decimals = 2 if probability < 10 else 1
            outcomes.append(f"{escape(outcome_title)} {fmt_num(probability, decimals)}%")
        details = [
            tr(
                "market.polymarket.liquidity",
                value=format_liquidity(liquidity),
            )
        ]
        end_date = str(event.get("endDate") or "")[:10]
        if end_date:
            details.append(tr("market.polymarket.closes", date=end_date))
        ...
        lines.extend(
            [
                "",
                f'<a href="{escape(event_url, quote=True)}">{escape(display_title)}</a>',
            ]
        )
        if outcomes:
            lines.append(" | ".join(outcomes))
        lines.append(" | ".join(details))
    return "\n".join(lines) if len(lines) > 1 else tr("market.polymarket.error")
