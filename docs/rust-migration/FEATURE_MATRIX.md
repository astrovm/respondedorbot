...
| Native typed OpenRouter non-streaming chat completions, tool calls, usage metadata, rate limits, malformed responses, and transport failures | `shadow` | Python provider orchestration remains authoritative until the complete native AI state machine, tools, streaming, and billing path passes parity | Rust adapter request/response and failure-path tests |
