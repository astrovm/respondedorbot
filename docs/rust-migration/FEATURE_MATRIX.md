...
| Native durable memory-compaction worker with compatible Redis jobs and leases, OpenRouter summary generation, provider-usage checkpointing before atomic summary/marker writes, legacy reservation settlement, durable crash recovery, stale-job settlement, bounded retry/refund, incompatible-job recovery, and unreadable-job quarantine | `shadow` | Python remains the deployed enqueue and worker owner until native foreground compaction planning/reservation and worker lifecycle are composed under the atomic runtime cutover | Rust fresh/recovered/obsolete/retry/terminal/lock/quarantine tests, typed OpenRouter request and billing-segment tests, strict workspace checks, and the existing Redis/PostgreSQL compatibility adapters |
