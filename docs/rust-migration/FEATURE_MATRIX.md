...
| Native Telegram AI draft delivery, first-token send, rate- and size-bounded edits, cleaned final edit, partial-draft cancellation, confirmed message identity, and delivery-gated settlement | `shadow` | Python remains the production poller; the native stream can produce side effects only inside the disabled native runtime and the existing Python runtime remains the deployment rollback | Rust stream-session, dispatcher integration, edit-failure, cancellation, cleanup, and delivery-acknowledgement tests |
| Native automatic AI image and audio context, Telegram file download, ffmpeg normalization, Redis-compatible cache, reserve extension, OpenRouter vision, free-then-paid Groq transcription with cooldown, OpenRouter transcription fallback, provider-segment accounting, and delivery-gated settlement | `shadow` | Python remains the production poller and media orchestrator until the native runtime cutover; the previous Python deployment remains the rollback while native media runs only inside the disabled runtime | Typed provider request/response tests, media pipeline cache/download/prepare/failure tests, real ffmpeg normalization test, provider fallback tests, conversation billing tests, polling-envelope tests, and production composition tests |
| Native `/transcribe` and `/describe` reply commands for voice, audio, video, video notes, photos, static stickers, and animated-sticker thumbnails, with exact bilingual replies, Telegram-safe text cleanup, cache-aware pricing, reserve adjustment, provider segment accounting, command-state writes, and delivery settlement | `shadow` | Python remains the deployed command owner until the atomic native polling cutover; missing native media composition leaves the command explicitly legacy-owned | Rust reply/help, audio, sticker, cached text, download failure, reserve/refund, dispatcher state, and delivery tests plus existing typed media/provider contracts |
| Native `/resumen`, `/summary`, and `/tldr` incremental summary streaming, custom instructions, compacted-marker history selection, cached/empty results, full-context reserve adjustment, summary usage accounting, Telegram-safe cleanup, command state, failure refunds, and delivery settlement | `shadow` | Python remains the deployed summary and compaction worker owner until the atomic runtime/worker cutover; the native summary path is reachable only inside the disabled native runtime | Rust prompt, marker, cache, empty-history, provider failure, streaming, billing, dispatcher state, and delivery tests plus Redis compatibility contracts |
| Native durable memory compaction with >40/keep-25 marker-aware foreground planning, full-context first compaction, post-delivery background reservation and compatible enqueue, lost-insert refund, Redis jobs and leases, OpenRouter summary generation, provider-usage checkpointing before atomic summary/marker writes, legacy settlement, crash recovery, stale-job settlement, bounded retry/refund, incompatible-job recovery, and unreadable-job quarantine | `shadow` | Python remains the deployed worker lifecycle owner until background services start and stop with the authoritative native polling runtime during the atomic cutover | Rust planning/context, delivery gating, reserve/enqueue/refund, fresh/recovered/obsolete/retry/terminal/lock/quarantine, typed OpenRouter, billing-segment, strict workspace, and Redis/PostgreSQL compatibility tests |
...
| Native declarative catalog for all 14 AI tools, exact provider schemas, availability filtering, scheduled-task restrictions, typed side-effect ports, and safe localized arithmetic execution | `shadow` | Python remains the deployed tool registry/executor while external tool ports and the complete native AI transaction are composed | Rust schema, availability, dispatch, arithmetic, localization, complexity, and malicious-input tests |
| Native typed external-tool request validation plus `calculate`, `bot_capabilities`, `random_choice`, and Firecrawl `web_search` execution, including bounded inputs, injected arbitrary-precision randomness, retries, localized failures, source output, and Firecrawl billing segments | `shadow` | Python remains the deployed AI/tool transaction owner until every tool and exact-once chat reservation/settlement are composed | Rust request-validation, pure catalog, random-source, Firecrawl transport, retry, localization, accounting, and failure tests |
...
| Native Solana/EVM address and `$ticker` token cards with typed DexScreener search/pair selection, GeckoTerminal OHLCV, pump.fun and Solana-supply enrichment, Python-readable Redis cache/state shapes and TTLs, annotated PNG candlestick rendering, normalized token-image fallback, HTML captions/social/trade links, native copy button, confirmed photo delivery, stock/market fallback, requester/admin-bound `sig:*` delete/refresh callbacks, cooldown, and multipart media edits | `shadow` | Python remains the deployed poller until the atomic runtime cutover; failed/no-data card attempts continue through the normal message route and every Rust cache value remains Python-readable for rollback | Core detection/selection/formatting/chart-eligibility tests, exact cache-shape and image tests, Telegram multipart send/edit tests, dispatcher delivery/state/fallback/authorization/cooldown/refresh tests, plus the existing Python token-signal suite |
...
| Native non-empty `/tarea`, `/tareas`, `/task`, and `/tasks` creation commands with future-task personal-credit preflight, localized failures, original-message persistence, provider-visible `task_set` instruction, AI tool execution, and delivery settlement | `shadow` | Python remains the deployed poller until the atomic runtime cutover; missing native AI composition remains an explicit startup/configuration failure | Rust dispatcher route, prompt isolation, credit preflight, provider suppression, AI tool, billing, and delivery tests plus existing Python command-routing tests |
