...
| Native private-chat `/language` and `/idioma` reads, validation, PostgreSQL updates, and inline keyboard | `shadow` | Group commands remain legacy-owned until Telegram-admin authorization and audit reporting move together | Core, dispatcher, and PostgreSQL adapter tests |
