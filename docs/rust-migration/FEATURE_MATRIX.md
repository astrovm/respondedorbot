...
| Telegram Bot API HTTP requests for messages, media uploads, edits, callbacks, payments, admin checks, typing, deletion, and command publication | `rust-flagged` | `RUST_TELEGRAM_HTTP_ADAPTER_ENABLED=0`, unsupported multipart shape, missing bridge, bridge error, or invalid bridge result | `contracts/telegram_http_adapter.json` |
