...
| Telegram non-multipart Bot API HTTP requests for messages, edits, callbacks, payments, admin checks, typing, deletion, and command publication | `rust-flagged` | `RUST_TELEGRAM_HTTP_ADAPTER_ENABLED=0`, multipart request, missing bridge, bridge error, or invalid bridge result | `contracts/telegram_http_adapter.json` |
